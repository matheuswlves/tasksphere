...
import { createProject, getProjectById, updateProject } from 'C:/Users/mathe/tasksphere/src/api/projectApi.js';
...
function ProjectFormPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditing && initialProjectCreatorId && user.id !== initialProjectCreatorId) {
        ...
        try {
            if (isEditing) {
                ...
                await updateProject(projectId, { ...currentProject, ...projectData });
            } else {
                await createProject({ ...projectData, creator_id: user.id, collaborators: [user.id] }); 
            }
    ...
    if (loading && isEditing) return <div>Carregando detalhes do projeto...</div>;
    if (isEditing && error.includes("autorizado")) {
        return <div style={{ color: 'red' }}>{error} <Link to="/dashboard">Voltar ao Dashboard</Link></div>;
    }
